...
#### 0.1) 补充：`sigma_alpha` 的物理含义（UNIFIED 模型的“角度粗糙度”，不是 nm 级 Ra）

- **定义（Geant4 UNIFIED）**：`sigma_alpha` 是“微表面法线相对宏观法线的角度偏离”的标准差（RMS），单位是 **rad**。  
  直观讲：它控制**微表面法线分布的展宽**，从而把原本严格的 Fresnel/TIR 条件“打散”，产生角度扩散（相当于微小的随机偏折）。

- **量级换算**：  
  \[
  0.1~\text{rad} \approx 5.73^\circ,\qquad 0.2~\text{rad} \approx 11.46^\circ
  \]
  因此 **`sigma_alpha=0.1` 不是一个“极小”的值**，它意味着微表面的 RMS 坡度（法线倾角）在几个度量级，足以显著破坏困光。

- **与真实表面粗糙度（高度）之间如何联系**：  
  `sigma_alpha` 更接近“**RMS slope**（坡度）”而不是“RMS height（高度）”。若要粗略把它映射成高度粗糙度，需要引入表面相关长度 \(L_c\)：  
  \[
  \text{RMS slope}\sim \frac{\sigma_h}{L_c}\quad\Rightarrow\quad \sigma_h\sim L_c\tan(\sigma_\alpha)
  \]
  例如：若 \(L_c\sim 10~\mu\text{m}\)，\(\sigma_\alpha=0.1\) 则 \(\sigma_h \sim 1~\mu\text{m}\) 量级；  
  若 \(L_c\sim 1~\mu\text{m}\)，则 \(\sigma_h \sim 0.1~\mu\text{m}\) 量级。  
  也就是说：**没有 \(L_c\) 就无法把 `sigma_alpha` 唯一对应到“多少 nm 粗糙度”**。

> 重要：本 `analytic_check2` 的 A/B/C 场景刻意把侧/顶墙面设为“镜面/朗伯”极限，因此仿真对照时建议设 `crystalSigmaAlpha=0`，避免把“晶体-空气界面的微粗糙”混进解析假设里。
...
## 5) Geant4 仿真对照（本仓库实际跑出的数值）

### 5.1 仿真口径与设置（对齐解析假设）

- **对照量**：\(\varepsilon_{\text{col}} = \texttt{Photon Hit si}/N\)（hit 到 `sipin_si` 就计数+kill；`pdetMode=0`）
- **光源**：550 nm 单色光（`g_debug_opticalphoton_wavelength=550 nm`），晶体中心各向同性发射（每事件 1 光子）
- **晶体自吸收**：`/SiPINLC/material/absorptionScale 10`（把 `ABSLENGTH` 统一 ×10，尽量弱化自吸收）
- **侧/顶墙面模型**：用 stepping override 实现（不改几何）：
  - `/SiPINLC/sanity/wallModel 1`：镜面（Specular）
  - `/SiPINLC/sanity/wallModel 2`：朗伯（Lambertian）
  - `/SiPINLC/sanity/wallReflectivity R`：每次撞墙反射概率 \(R\)，否则视为墙面吸收（计入 `Escaped_PTFE`）
  - 只对 side/top 生效，底面不覆盖（保留 air/grease 底耦合的 Fresnel/TIR）
- **关闭其他 override**：`/SiPINLC/geometry/sideContactRatio 0`，`/SiPINLC/geometry/crystalSigmaAlpha 0`
- **数值稳定性**：对场景 A（specular + TIR 困光）启用 `/SiPINLC/sanity/maxSteps` 安全阈值，超过步数直接判为“lost”（计入 `Escaped_Other`），用于避免超长轨迹导致运行时间爆炸。

对应宏文件（每个场景 `N=1e3`）：
- `mac/analytic_check2_A_air.mac`
- `mac/analytic_check2_A_grease.mac`
- `mac/analytic_check2_B_air.mac`
- `mac/analytic_check2_B_grease.mac`
- `mac/analytic_check2_C_air.mac`
- `mac/analytic_check2_C_grease.mac`

### 5.2 仿真结果（待跑完后回填）

| 场景 | bottom medium | wall model | R | N | \(\varepsilon_{\text{col}}\) (Geant4) | Escaped_PTFE/N | 备注 |
|---|---|---|---:|---:|---:|---:|---|
| A | air | specular | 1.000 | 1e6 | TBD | TBD | |
| A | grease | specular | 1.000 | 1e6 | TBD | TBD | |
| B | air | lambertian | 1.000 | 1e6 | TBD | TBD | |
| B | grease | lambertian | 1.000 | 1e6 | TBD | TBD | |
| C | air | lambertian | 0.975 | 1e6 | TBD | TBD | |
| C | grease | lambertian | 0.975 | 1e6 | TBD | TBD | |
